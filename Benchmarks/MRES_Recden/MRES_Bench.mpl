...
polygen:= proc(c, nvar::nonnegint, tdeg::nonnegint, gcddeg::nonnegint,
                degfield, numext)
    uses RDP = Algebraic:-RecursiveDensePolynomials;
    local mins, Xmin, Xpoly, XT, pickSS, a, b,m,i;
    ...
    return a, b, mins, Xpoly;
end proc:
##########################################################################################
#flagalg:=MRES1     #uses (RDP=Algebrai:-RecursiveDensePolynomials version) to compute the gcd using the Primitive element
#flagalg:=MRES2     #uses (RDP=Algebrai:-RecursiveDensePolynomials version) to compute the gcd without using the Primitive element
#########################################################################################
...
Benchmain:=proc(flagalg::Or(function,procedure),
 ...
 uses RDP = Algebraic:-RecursiveDensePolynomials;
local c,a0,b0,g0,header, R,f1,f2,F1,F2,t,k,G,TA,ncols,i,j,d,timings,Indx,var,mins,Args,lb,ub,counter,S,dg,G0,Nmin,Nrange,Range,J,T,Np,Xpoly;
#we use randomize to create pseudo- random polys in polygen and Minpolylist
   ...
   header := ["dF", "#ext", "#var",
    ...
    for i in Range do
        ...
        F1,F2,mins,Xpoly:=polygen(c,S);
        CodeTools:-Profiling:-Profile(flagalg);  
        try  
            t := time():
               G := timelimit(100, flagalg(F1,F2,Xpoly[1]));
            TA  :=time()-t; 
            Np:=G[2];
        ...
        R[j]:= Vector([ d, Nmin, var, 
             RDP:-tdegrpoly(F1), RDP:-tdegrpoly(F2), 
             dg,  TA, Np ]);
        j:=j+1;        
        end do;
   end do;
